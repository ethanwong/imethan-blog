...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addMulti = function(rowid,pos,irow,checked){
			var	v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + (checked ? "checked=\"checked\"" : "")+"/><span style='margin-left:7px;padding-left:14px;'></span>",
			...
			if(pgboxes) {
				...
				if(ts.p.pgbuttons===true) {
					...
					if(cp===1 || cp === 0) {
						...
						$("#first"+tspg+"_first, #prev"+tspg+"_prev").attr('disabled',"true");
						if(ts.p.toppager) { $("#first_t"+tspg_t+", #prev_t"+tspg_t).addClass('ui-state-disabled').removeClass('ui-state-hover'); }
					} else {
						$("#first"+tspg+", #prev"+tspg).removeClass('ui-state-disabled');
						$("#first"+tspg+"_first, #prev"+tspg+"_prev").removeAttr('disabled');
						if(ts.p.toppager) { $("#first_t"+tspg_t+", #prev_t"+tspg_t).removeClass('ui-state-disabled'); }
					}
					if(cp===last || cp === 0) {
						$("#next"+tspg+", #last"+tspg).addClass('ui-state-disabled').removeClass('ui-state-hover');
						$("#next"+tspg+"_next, #last"+tspg+"_last").attr('disabled',"true");
						if(ts.p.toppager) { $("#next_t"+tspg_t+", #last_t"+tspg_t).addClass('ui-state-disabled').removeClass('ui-state-hover'); }
					} else {
						$("#next"+tspg+", #last"+tspg).removeClass('ui-state-disabled');
						$("#next"+tspg+"_next, #last"+tspg+"_last").removeAttr('disabled');
...
//				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-end'></span></td>";
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><button id='"+po[0]+"_first' class='btn btn-xs btn-default-outline' style='margin-top: 7px;'>首页</button></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><button id='"+po[1]+"_prev' class='btn btn-xs btn-default-outline' style='margin-top: 7px;'>上一页</button></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><button id='"+po[2]+"_next' class='btn btn-xs btn-default-outline' style='margin-top: 7px;margin-left: 9px;'>下一页</button></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><button id='"+po[3]+"_last' class='btn btn-xs btn-default-outline' style='margin-top: 7px;'>尾页</button></td>";
		...
		if(this.p.multiselect) {
			this.p.colNames.unshift("<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox'/><span id='cb_"+this.p.id+"span' style='margin-left:23px;'></span>");
		...
		if(this.p.multiselect) {
			var emp=[], chk;
			$('#cb_'+$.jgrid.jqID(ts.p.id)+"span",this).bind('click',function(){
//				alert($('#cb_'+$.jgrid.jqID(ts.p.id)).prop("checked"));
				ts.p.selarrrow = [];
				var froz = ts.p.frozenColumns === true ? ts.p.id + "_frozen" : "";
				if (!$('#cb_'+$.jgrid.jqID(ts.p.id)).prop("checked")) {
					...
					chk=true;
					emp=[];
					$('#cb_'+$.jgrid.jqID(ts.p.id)).prop("checked", true); 
				}
				else {
					$(ts.rows).each(function(i) {
						if(i>0) {
					...
					ts.p.selrow = null;
					chk=false;
					$('#cb_'+$.jgrid.jqID(ts.p.id)).prop("checked", false); 
				}
...
//Helper functions
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		cnt += c_b ? "<div class='ui-widget-content ui-helper-clearfix' style='text-align:"+mopt.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'><a href='javascript:void(0)' id='closedialog' class='fm-button btn btn-xs  btn-default-outline'>"+c_b+"</a>"+buttstr+"</div>" :
		...
		if (editable==="1") {
			...
			if (cv[0] === false){
				try {
					...
					$.jgrid.info_dialog($.jgrid.errors.errcap,cv[1],$.jgrid.edit.bClose,{left:positions[0]+245,top:positions[1]+$(tr).outerHeight()});
				} catch (e) {
					alert(cv[1]);
				}
				return success;
